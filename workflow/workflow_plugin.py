...
ACTIONS_POINT = u'enaml.workbench.workflow.actions'

APPLICATION_FACTORY_POINT = u'enaml.workbench.workflow.application_factory'

BRANDING_POINT = u'enaml.workbench.workflow.branding'

WINDOW_FACTORY_POINT = u'enaml.workbench.workflow.window_factory'

WORKNODES_POINT = u'enaml.workbench.workflow.worknodes'

AUTOSTART_POINT = u'enaml.workbench.workflow.autostart'
...
class WorkflowPlugin(Plugin):
    ...
    @property
    def worknode(self):
        """ Get a reference to the currently active worknode.

        """
        return self._model.worknode
    ...
    def close_worknode(self):
        """ Close and dispose of the currently active worknode.

        """
        self._worknode_extension = None
        self._model.worknode.stop()
        self._model.worknode.workbench = None
        self._model.worknode = Worknode()

    def select_worknode(self, extension_id):
        """ Select and start the worknode for the given extension id.

        The current worknode will be stopped and released.

        """
        target = None
        workbench = self.workbench
        point = workbench.get_extension_point(WORKNODES_POINT)
        ...
        if target is None:
            msg = "'%s' is not a registered worknode extension"
            raise ValueError(msg % extension_id)

        if target is self._worknode_extension:
            return

        old_worknode = self._model.worknode
        old_worknode.stop()
        old_worknode.workbench = None

        self._worknode_extension = target
        new_worknode = self._create_worknode(target)

        new_worknode.workbench = workbench
        new_worknode.start()
        self._model.worknode = new_worknode
    ...
    _window = Typed(WorkflowWorkbenchWindow)

    #: The view model object used to drive the window.
    _model = Typed(WorkflowWindowModel)

    #: The currently activate branding extension object.
    _branding_extension = Typed(Extension)

    #: The currently activate worknode extension object.
    _worknode_extension = Typed(Extension)
    ...
    def _create_model(self):
        ...
        """
        self._model = WorkflowWindowModel()
    ...
    def _create_window(self):
        ...
        if not isinstance(window, WorkflowWorkbenchWindow):
    ...
    def _create_worknode(self, extension):
        """ Create the worknode object for the given extension.
        ...
        -------
        result : Worknode
            The worknode object for the given extension.

        """
        if extension.factory is None:
            msg = "extension '%s' does not declare a worknode factory"
            raise ValueError(msg % extension.qualified_id)

        worknode = extension.factory(self.workbench)
        if not isinstance(worknode, Worknode):
            msg = "extension '%s' created non-worknode type '%s'"
            args = (extension.qualified_id, type(worknode).__name__)
            raise TypeError(msg % args)
